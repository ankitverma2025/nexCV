    ...
    const fetchAllData = async () => {
      try {
        const [profileData, expData, eduData, skillsData, projectsData] = await Promise.all([
          ...
          experienceApi.getExperiences(username).catch(() => []),
          educationApi.getEducation(username).catch(() => []),
          skillsApi.getSkills(username).catch(() => []),
          projectsApi.getProjects(username).catch(() => []),
        ]);
